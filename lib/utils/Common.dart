...
Future logout(BuildContext context) async {
  return showConfirmDialogCustom(context,
      ...
      negativeText: AppLocalizations.of(context)!.translate('lbl_cancel'),
      onAccept: (context) {
...
Future<void> setDynamicStatusBarColor({Color? color, int milliseconds = 100}) async {
  if (getIntAsync(DASHBOARD_PAGE_VARIANT, defaultValue: 1) == 1) {
    setStatusBarColor(color ?? primaryColor!, delayInMilliSeconds: milliseconds);
...
class MyLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Image.asset(app_logo, height: 30, width: 30));
  }
}
